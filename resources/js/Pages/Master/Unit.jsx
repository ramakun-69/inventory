...
export default function Unit({ ...props }) {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.units'), {
            params: {
                page: currentPage,
    ...
    const handleShowModal = (supplier = null) => {
        ...
        setModal({
            show: true,
            title: supplier ? t('Edit Unit') : t('Add New Unit'),
        });
    }
    ...
    const handleDelete = (id) => {
        confirmAlert(t('Are You Sure?'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.units.destroy', id), {
                onSuccess: (page) => {
                    const error = page.props?.flash?.error;
                    const success = page.props?.flash?.success;
                    if (error) notifyError(error, 'bottom-center');
    ...
    return (
        <AppLayout>
            <Breadcrumb title={t('Unit')} subtitle={t('Unit Management')} />
            <div className="container">
                <div className="d-flex justify-content-end mb-3">
                    ...
                    <Link href={route('trash.units')} className="btn btn-sm btn-danger ms-2">
            ...
            >
                <div className="row">
                    <div className="mb-3">
                        <label htmlFor="name" className="form-label">{t('Unit Name')}</label>
                        <TextInput
                            id="name"
                            type="text"
                            ...
                            placeholder={t('Enter Attribute', { 'attribute': t('Unit Name') })}
                            value={data.name}
