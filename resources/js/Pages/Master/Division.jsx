...
export default function Division({ ...props }) {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.divisions'), {
            params: {
                page: currentPage,
    ...
    const handleShowModal = (division = null) => {
        division ? setData({ ...division, division_name: division.name }) : reset();
        setModal({
            show: true,
            title: division ? t('Edit Division') : t('Add New Division'),
        });
    }
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        clearErrors();
        post(route('master-data.divisions.store'), {
            onSuccess: (page) => {
    ...
    const handleDelete = (id) => {
        confirmAlert(t('Are You Sure?'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.divisions.destroy', id), {
                onSuccess: (page) => {
                    const error = page.props?.flash?.error;
                    const success = page.props?.flash?.success;
                    if (error) notifyError(error, 'bottom-center');
        ...
        <AppLayout>
            ...
            <div className="container">
                <div className="d-flex justify-content-end mb-3">
                    ...
                    <Link href={route('trash.divisions')} className="btn btn-sm btn-danger ms-2">
            ...
            >
                <div className="row">
                    <div className="mb-3">
                        <label htmlFor="division_name" className="form-label">{t('Division Name')}</label>
                        <TextInput
                            id="division_name"
                            type="text"
                            ...
                            placeholder={t('Enter Attribute', { 'attribute': t('Division Name') })}
