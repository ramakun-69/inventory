...
export default function Category({ ...props }) {
    ...
    const handleShowModal = (supplier = null) => {
        ...
        setModal({
            show: true,
            title: supplier ? t('Edit Category') : t('Add New Category'),
        });
    }
    ...
    const handleDelete = (id) => {
        confirmAlert(t('Are You Sure?'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.categories.destroy', id), {
                onSuccess: (page) => {
                    const error = page.props?.flash?.error;
                    const success = page.props?.flash?.success;
                    if (error) notifyError(error, 'bottom-center');
        ...
        <AppLayout>
            ...
            <div className="container">
                <div className="d-flex justify-content-end mb-3">
                    ...
                    <Link href={route('trash.categories')} className="btn btn-sm btn-danger ms-2">
            ...
            >
                <div className="row">
                    <div className="mb-3">
                        <label htmlFor="name" className="form-label">{t('Category Name')}</label>
                        <TextInput
                            id="name"
                            type="text"
                            ...
                            placeholder={t('Enter Attribute', { 'attribute': t('Category Name') })}
                            value={data.name}
