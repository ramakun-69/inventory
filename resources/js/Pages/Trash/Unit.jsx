...
export default function Unit({ ...props }) {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.trash.units'), {
            params: {
                page: currentPage,
    ...
    const handleDelete = () => {
        confirmAlert(t('Are You Sure?'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.units.delete'), {
                onSuccess: (page) => {
                    reset();
    ...
    const handleRestore = () => {
        confirmAlert(t('Are You Sure?'), t('restore_description'), 'warning', () => {
            post(route('master-data.units.restore'), {
                onSuccess: (page) => {
                    reset();
                    const error = page.props?.flash?.error;
                    const success = page.props?.flash?.success;
        ...
        <AppLayout>
            ...
            <div className="container">
                <div className="d-flex justify-content-end mb-3">
                    ...
                    <Link href={route('master-data.units.index')} className="btn btn-sm btn-secondary" >
