...
export default function Category({ ...props }) {
    ...
    const loadTableData = () => {
        setIsLoading(true);
        axios.get(route('datatable.trash.categories'), {
            params: {
                page: currentPage,
    ...
    const handleDelete = () => {
        confirmAlert(t('Are You Sure?'), t('delete_description'), 'warning', () => {
            destroy(route('master-data.categories.delete'), {
                onSuccess: (page) => {
                    reset();
    ...
    const handleRestore = () => {
        confirmAlert(t('Are You Sure?'), t('restore_description'), 'warning', () => {
            post(route('master-data.categories.restore'), {
                onSuccess: (page) => {
                    reset();
                    const error = page.props?.flash?.error;
                    const success = page.props?.flash?.success;
        ...
        <AppLayout>
            ...
            <div className="container">
                <div className="d-flex justify-content-end mb-3">
                    ...
                    <Link href={route('master-data.categories.index')} className="btn btn-sm btn-secondary" >
